...
class PurchaseRequisitionController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($purchase_requisitions)
                ->addColumn('confirmedUsers', function ($row) {
                    $transactionApprovals = \Illuminate\Support\Facades\DB::table('transaction_approvals')
                        ->select('user_id')
    ...
    public function store(PurchaseRequisitionRequest $request, CreatePurchaseRequisition $createPurchaseRequisition): RedirectResponse
    {
        if (!auth()->user()->can('purchase_requisition.create')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            $createPurchaseRequisition->execute($request->validated());
        ...
        return redirect()->route('purchase-requisition.index')->with('status', $output);
    }
    ...
    public function getRequisitionProducts()
    {
        if (request()->ajax()) {
            $business_id = request()->session()->get('user.business_id');
            if (!empty(request()->input('sale_order_ref_no'))) {
                $ref_no = request()->input('sale_order_ref_no');
                $trans = Transaction::where('invoice_no', $ref_no)
            ...
            $q = TransactionSellLine::where('transaction_id', $transaction_id);


            $sell_lines = $q->get();
            $variation_ids = $q->select('variation_id')->pluck('variation_id')
                ->toArray();
            ...
            return view('purchase_requisition.product_list')->with(compact('products', 'sell_lines'));
        }
    }
    ...
    public function getPurchaseRequisitions($location_id)
    {
        ...
        $purchase_requisitions = Transaction::where('business_id', $business_id)
            ...
            ->whereDoesntHave('transactionApprovals', function ($query) {
