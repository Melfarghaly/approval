...
class TransactionUtil extends Util
{
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if($payment['method']=='cash'){
                        $payment['accounting_account_id']=$payment['cash_account_id'];
                    }elseif(in_array($payment['method'],['cheque','bank_transfer','card'])){
                        $payment['accounting_account_id']=$payment['bank_account_id'];
                    }
                    $payment_data = [
    ...
    public function isApproved($type, $user_id) {
        // Define the approval array
        $approval_array = session('business.approval_counter');
        \DB::table('notifications')->insert([
            'id' => \Str::uuid()->toString(), // Generate a unique UUID for the notification ID
            'type' => 'App\Approval', // Customize this based on your use case
            'notifiable_type' => 'App\User', // Adjust to match your User model's namespace
            'notifiable_id' => $user_id,
            'data' => json_encode([
                'message' => "Transaction of type '{$type}' has been approved by user ID {$user_id}. Please review and approve it.",
                'type' => $type,
                'approved_by' => $user_id,
            ]),
            'created_at' => now(),
        ...
        if (!isset($approval_array[$type])) {
            return response()->json(['error' => 'Invalid transaction type'], 400);
        }
    
        // Get the approvers for this type
        $approvers = $approval_array[$type];
    
        // Find the current user's position in the approval chain
        $currentIndex = array_search($user_id, $approvers);
    
        if ($currentIndex === false) {
            //return response()->json(['error' => 'User not in approval chain'], 400);
        }
    
        // Determine the next approver
        $nextIndex = $currentIndex + 1;
        $nextUserId = $approvers[$nextIndex] ?? null;
    
        if ($nextUserId === null) {
            // All approvals completed
            return response()->json(['message' => 'Transaction fully approved'], 200);
        }
        
        // Notify the next user
        $nextUserExists = \DB::table('users')->where('id', $nextUserId)->exists();
        if ($nextUserExists) {
            // Create a new notification
            \DB::table('notifications')->insert([
                'id' => \Str::uuid()->toString(), // Generate a unique UUID for the notification ID
                'type' => 'App\Notifications\ApprovalNotification', // Customize this based on your use case
                'notifiable_type' => 'App\Models\User', // Adjust to match your User model's namespace
                'notifiable_id' => $nextUserId,
                'data' => json_encode([
                    'message' => "Transaction of type '{$type}' has been approved by user ID {$user_id}. Please review and approve it.",
                    'type' => $type,
                    'approved_by' => $user_id,
            ...
            return response()->json(['message' => 'Notification sent to the next approver'], 200);
        }
    
        return response()->json(['error' => 'Next user not found'], 404);
    }
    function storeApproval($user_id, $transaction_id) {
        // Validate inputs
        if (empty($user_id) || empty($transaction_id)) {
            return [
                'success'=>false,
                'msg' => 'User ID and Transaction ID are required'
            ];
             
        }

        // Check if the transaction already exists in approvals for this user
...
/*
        if ($existingApproval) {
            return [
                ...
                'success'=>false,
                'msg' => 'Approval already recorded for this transaction'
            ];
...
*/
        ...
        $transaction=Transaction::find($transaction_id);
        $this->activityLog($transaction, 'approved');
        $this->isApproved($transaction->type, $user_id);
        return [
            'success'=>true,
            'msg' => 'Approval recorded successfully'];
    }
    function need_approve($type, $user_id, $transaction_id) {
        // Define the approval array
        $approval_array = session('business.approval_counter');
        // Validate the transaction type
        if (!isset($approval_array[$type])) {
        ...
        // Get the approval chain for the transaction type
        $approvers = $approval_array[$type];
    
        // Loop through the approvers and find the first unapproved user for the transaction
        foreach ($approvers as $approver_id) {
            // Skip if approver ID is null
            if ($approver_id === null) {
                continue;
            }
    
            // Check if this approver has already approved the transaction
            ...
            if (!$isApproved) {
                // If this is the first unapproved user, check if it matches the given user ID
